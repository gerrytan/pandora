...
func PossibleValuesForBackupDestinationType() []string {
	return []string{
		string(BackupDestinationTypeAZURE),
		string(BackupDestinationTypeOCI),
	}
}

func (s *BackupDestinationType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseBackupDestinationType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseBackupDestinationType(input string) (*BackupDestinationType, error) {
	vals := map[string]BackupDestinationType{
		"azure": BackupDestinationTypeAZURE,
		"oci":   BackupDestinationTypeOCI,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := BackupDestinationType(input)
	return &out, nil
}
