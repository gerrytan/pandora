...
func PossibleValuesForProximityPlacementGroupEntityType() []string {
	return []string{
		string(ProximityPlacementGroupEntityTypeCloudExadataInfrastructure),
		string(ProximityPlacementGroupEntityTypeOtherProducts),
	}
}

func (s *ProximityPlacementGroupEntityType) UnmarshalJSON(bytes []byte) error {
	var decoded string
	if err := json.Unmarshal(bytes, &decoded); err != nil {
		return fmt.Errorf("unmarshaling: %+v", err)
	}
	out, err := parseProximityPlacementGroupEntityType(decoded)
	if err != nil {
		return fmt.Errorf("parsing %q: %+v", decoded, err)
	}
	*s = *out
...
func parseProximityPlacementGroupEntityType(input string) (*ProximityPlacementGroupEntityType, error) {
	vals := map[string]ProximityPlacementGroupEntityType{
		"cloudexadatainfrastructure": ProximityPlacementGroupEntityTypeCloudExadataInfrastructure,
		"otherproducts":              ProximityPlacementGroupEntityTypeOtherProducts,
	}
	if v, ok := vals[strings.ToLower(input)]; ok {
		return &v, nil
	}
	...
	// otherwise presume it's an undefined value and best-effort it
	out := ProximityPlacementGroupEntityType(input)
	return &out, nil
}
